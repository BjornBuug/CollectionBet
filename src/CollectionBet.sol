...
contract CollectionBet is
    EIP712("CollectionBet", "1"),
    Ownable,
    ReentrancyGuard,
    ERC721TokenReceiver
{
    ...
    event UpdateMinimumValidNonce(
        address indexed orderMaker,
        uint256 minimumNonceOrder
    );
    event UpdateMinimumValidNonceSell(
        address indexed orderMaker,
        uint256 minimumNonceOrderSell
    );
     ...
     * @param sellerDeposit Amount deposited by the seller, Non refundable fee for entrenting the contract.
    ...
    bytes32 public constant ORDER_TYPE_HASH =
        keccak256(
            "Order(uint256 sellerDeposit, uint256 buyerCollateral, uint256 validity, uint256 expiry, uint256 nonce, uint16 fee, address maker, address paymentAsset, address collection, bool isBull)"
        );
    ...
    bytes32 public constant SELL_ORDER_TYPES_HASH =
        keccak256(
            "SellOrder(bytes32 orderHash, uint256 price, uint256 start, uint256 end, uint256 nonce, address paymentAsset, address maker, address[] whitelist, bool isBull)"
        );
    ...
    function matchOrder(
        ...
        uint256 makerTokensBalance = IERC20(tokenPayment).balanceOf(
            order.maker
        );
     ...
     */
    function batchSettleContract(
        Order[] calldata orders,
        uint[] calldata tokensIds
    ) external {
    ...
    function buyPosition(
        ...
        require(
            sellOrder.whitelist.length == 0 ||
                isWhiteListed(sellOrder.whitelist, msg.sender),
        ...
        if (msg.value > 0) {
            ...
            IERC20(paymentAsset).safeTransferFrom(
                msg.sender,
                sellOrderMaker,
                buyerPrice
            );
    ...
    function isValidSellOrder(
        ...
        require(
            isValidSignature(sellOrder.maker, sellOrderHash, signature),
            "INVALIDsignature"
        );
        ...
        if (sellOrder.isBull) {
            require(
                sellOrder.maker == buyers[contractId],
                "CALLER_IS_NOT_A_BUYER"
            );
            require(!reclaimedNFT[contractId], "ALREADY_RECLAIMED");
        } else {
        ...
        require(
            confirmedSellOrder[sellOrderHash].maker == address(0),
            "ORDER_ALREADY_SOLD"
        );
        ...
        require(
            sellOrder.nonce >= minimumValidNonceOrderSell[sellOrder.maker],
            "INVALID_NONCE"
        );
    }
    ...
    function cancelSellOrder(SellOrder calldata sellOrder) external {
        ...
        require(
            confirmedSellOrder[hashedSellOrder].maker == address(0),
            "ALREADY_SOLD"
        );
    ...
    function cancelOrder(Order calldata order) external {
        ...
        require(
            matchedOrders[uint256(hashedOrder)].maker == address(0),
            "ALREADY_MATCHED"
        );
    ...
    function isValidOrder(
        ...
        require(
            isValidSignature(order.maker, _orderHash, _signature),
            "INVALID_SIGNATURE"
        );
        ...
        require(
            matchedOrders[uint256(_orderHash)].maker == address(0),
            "ORDER_ALREADY_MATCHED"
        );

        // Add more conditions to check the order is valid
        require(
            order.nonce >= minimumValidNonceOrder[order.maker],
            "INVALID_NONCE"
        );
     ...
     */
    function setMinimumValidNonceOrderSell(
        uint256 _minimumValidNonceOrderSell
    ) external {
        // Create a mapping address => nonce to keep track of each Order.maker setting invalid orders
        require(
            _minimumValidNonceOrderSell >
                minimumValidNonceOrderSell[msg.sender],
            "NONCE_TO_LOW"
        );

        minimumValidNonceOrderSell[msg.sender] = _minimumValidNonceOrderSell;

        emit UpdateMinimumValidNonceSell(
            msg.sender,
            _minimumValidNonceOrderSell
        );
    }
    ...
    function setMinimumValidNonce(uint256 _minimumValidNonceOrder) external {
        // Create a mapping address => nonce to keep track of each Order.maker setting invalid orders
        require(
            _minimumValidNonceOrder > minimumValidNonceOrder[msg.sender],
            "NONCE_TO_LOW"
        );
