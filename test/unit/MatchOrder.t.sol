...
contract TestMatchOrder is Base {
    ...
    */
    function test_OrderMatchsWithSufficientWETHBalance() public {
        ...
        uint256 sellerDeposit = (order.sellerDeposit * fee) /
            1000 +
            order.sellerDeposit;
        uint256 buyerCollateral = (order.buyerCollateral * fee) /
            1000 +
        ...
        assertEq(
            contractBalInWETH,
            sellerDeposit + buyerCollateral,
            "Contract should hold the right amount of WETH"
        );
    }
    ...
    */
    function test_CannotSendETHIfAllowedTokenIsNotWETH() public {
        ...
        uint takerPrice = (order.sellerDeposit * fee) /
            1000 +
            order.sellerDeposit;
    ...
    */
    ...
    function test_MatchOrderStructSaved() public {
        ...
        ) = nftLongShortTrade.matchedOrders(uint(hashedOrder));
    ...
    function test_contractBalancePostFeesWithdraw() public {
        ...
        assertEq(
            weth.balanceOf(address(nftLongShortTrade)),
            sellerDeposit + buyerCollateral
        );
    }
    ...
    function test_WETHDepositedWhenETHIsSent() public {
        ...
        uint fee = nftLongShortTrade.fee();
        uint sellerDepositInEth = (order.sellerDeposit * fee) /
            1000 +
            order.sellerDeposit;
        uint256 buyerCollateral = (order.buyerCollateral * fee) /
            1000 +
        ...
        nftLongShortTrade.matchOrder{value: sellerDepositInEth}(
            order,
            signature
        );
        ...
        assertEq(
            weth.balanceOf(address(nftLongShortTrade)),
            sellerDepositInEth + buyerCollateral
        );
    }
}
